...
// Catálogo estático de fallback para asegurar usabilidad del MVP
...
export default function NuevoPedidoPage() {
  ...
  const [selectedWeightServiceId, setSelectedWeightServiceId] = useState<string>('s1');
    ...
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.trim().length >= 2) {
        try {
          ...
          // Si Supabase no está conectado, simulamos datos de prueba
          if (results.length === 0) {
            setSearchResults([
  ...
  const handleCreateCustomerSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Intentamos llamar a la Server Action de Supabase
      ...
      if (res.success && res.data) {
        ...
        // Fallback local si Supabase no está configurado aún en este paso
        setSelectedCustomer({
          id: 'c-local-new',
          full_name: newCustomerName.trim(),
          phone: newCustomerPhone.trim() || null,
  ...
  const getWeightServicePrice = () => {
    const service = FALLBACK_SERVICES.find(s => s.id === selectedWeightServiceId);
    return service ? (service.price_per_kg || 0) : 0;
  };

  const getSpecialItemPrice = () => {
    if (!selectedSpecialItemId) return 0;
    const item = FALLBACK_SPECIAL_ITEMS.find(e => e.id === selectedSpecialItemId);
  ...
  const handleRegisterOrder = async () => {
    ...
    if (weightKg > 0) {
      orderItems.push({
        service_id: '00000000-0000-0000-0000-000000000000', // Marcador temporal
        description: 'Servicio por peso',
        quantity: 1,
        weight_kg: weightKg,
    ...
    if (selectedSpecialItemId) {
      orderItems.push({
        service_id: '00000000-0000-0000-0000-000000000000', // Marcador temporal
        description: FALLBACK_SPECIAL_ITEMS.find(e => e.id === selectedSpecialItemId)?.name || 'Artículo especial',
    ...
    try {
      // Mock de ID de empleada recibidora (Esposa del dueño para desarrollo)
      ...
      const payload = {
        customer_id: selectedCustomer.id === 'quick-id-placeholder' || selectedCustomer.id === 'c-local-new' 
          ? '00000000-0000-0000-0000-000000000000' // ID temporal si no hay base
          : selectedCustomer.id,
      ...
      // Intentar disparar Server Action real
      await createOrder(payload as any);
      alert('¡Pedido registrado y cobrado con éxito!');
      router.push('/');
    } catch (err) {
      // Fallback de simulación exitosa en desarrollo local
    ...
    <div className={styles.container}>
      ...
      <section className={`${styles.section} glassmorphism`}>
        ...
        <div style={{ marginBottom: 'var(--spacing-lg)' }}>
          ...
          <div className={styles.weightServiceSelector}>
            {FALLBACK_SERVICES.map((s) => (
              ...
              >
                {s.name.replace(' x Kilo', '')} (S/ {s.price_per_kg?.toFixed(2)})
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <ChipSelector
            options={FALLBACK_SPECIAL_ITEMS}
      ...
      {/* RESUMEN FINAL Y BOTÓN DE REGISTRO */}
      <div className={`${styles.footerSummary} glassmorphism`}>
